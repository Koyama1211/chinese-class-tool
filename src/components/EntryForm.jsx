...
function parseAiOutput(text) {
  ...
  return {
    chinese: (sections.chinese || []).join('\n'),
    pinyin: (sections.pinyin || []).join('\n'),
    japanese: (sections.japanese || []).join('\n'),
  }
}
...
export default function EntryForm({ initial, onSave, onCancel }) {
  ...
  function handleParse() {
    const parsed = parseAiOutput(pasteText)
    if (!parsed.chinese && !parsed.pinyin && !parsed.japanese) {
  ...
  return (
    <div className="entry-form">
      <div className="tab-bar">
        <button className={`tab ${tab === 'manual' ? 'active' : ''}`} onClick={() => setTab('manual')}>手動入力</button>
      ...
      {tab === 'ai' && (
        <div className="ai-panel">
          ...
          <button className="btn-primary" onClick={handleParse}>解析して入力欄に反映</button>
        </div>
      )}
